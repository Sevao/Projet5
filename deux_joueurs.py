...
with open("liste_mots.txt", "r") as f:
    mots = [mot.strip().lower() for mot in f if len(mot.strip()) == 7]
...
def valider():
    ...
    if len(mot) != 7 or not mot.isalpha():
        ...
        entree.delete(0, tk.END)
        return # on sort de la fonction avant de reset le timer

    arreter_timer()  # Arrêter le timer seulement si le mot est valide
...
# Interface tekinter
fenetre = tk.Tk()
fenetre.title("Motus simplifié - 2 joueurs")
